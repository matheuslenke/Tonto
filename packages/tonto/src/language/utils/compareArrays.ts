...
export function compareArrayWithSet(array1: OntologicalNature[], set: Set<OntologicalNature>): boolean {
    if (array1.length !== set.size) {
        return false;
    }

    // Sort the arrays to ensure consistent comparison
    const sortedArray1 = array1.sort();
    const sortedArray2 = Array.from(set).sort();
