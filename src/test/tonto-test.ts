...
export function parseHelper<T extends AstNode = AstNode>(services: TontoServices): (input: string) => Promise<LangiumDocument<T>> {
...
export function validationHelper<T extends AstNode = AstNode>(services: TontoServices): (input: string) => Promise<ValidationResult<T>> {
